...
export function Login() {
  const { signIn ,isLoading , signUpWithGoogle , signUpWithGithub} = useAuthStore();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => await signIn(data);
  

  //! Render
  const renderInputs = LOGIN_DATA.map((input) => (
    ...
    <div className="flex min-h-screen items-center justify-center">
      <div className="shadow-input  w-full max-w-md rounded-none  p-4 md:rounded-2xl  bg-black">
        ...
        <form className="my-8" onSubmit={handleSubmit(onSubmit)}>
          ...
          <p className="text-center mt-2 text-sm text-neutral-700 dark:text-neutral-300">You don't have an account? 
            <Link to="/register" className="text-[#1DCD9F]"> Sign Up</Link>
          </p>
        </form>
      </div>
    </div>
